...
//                                                                                           1, 25));
    ...
    void render5(std::shared_ptr<VirtualObject> object, std::string filename, const uint32_t w, const uint32_t h,const uint32_t samples) {


        const auto sampler = generateSampler(samples);

        auto selectedWorld = worlds::buildingsScene4(object);
